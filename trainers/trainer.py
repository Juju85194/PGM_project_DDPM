...
from utils.helpers import compare_schedules
import os
...
class Trainer:
    ...
    def train(self):
        compare_schedules(self.config)

        for schedule_fn in self.config.beta_schedule_fns:
            print(f"Training with schedule: {schedule_fn.__name__}")
            self.betas = schedule_fn(timesteps=self.timesteps)
            ...
            for epoch in range(self.config.epochs):
                ...
                for step, batch in enumerate(self.dataloader):
                    ...
                    if step % self.config.log_every == 0:
                        print(f"Loss ({schedule_fn.__name__}):", loss.item())
                    ...
                    if step != 0 and step % self.config.save_and_sample_every == 0:
                        ...
                        save_image(all_images, str(self.results_folder / f'sample-{schedule_fn.__name__}-{milestone}.png'), nrow = 6)
                        ...
                        save_gif(f'diffusion-{schedule_fn.__name__}-{milestone}.gif', samples, self.config.image_size, self.config.channels)

            # Save the model for the current schedule
            torch.save(self.denoise_model.state_dict(), os.path.join(self.results_folder, f"model_{schedule_fn.__name__}.pth"))